...
exports.master = (path_to_worker, supplied_config, path_to_config) ->
  ...
  supplied_config = if supplied_config? then supplied_config else {}
  for key of supplied_config
    config[key] = supplied_config[key]
  ...
  process.on 'SIGUSR2', () ->
    ...
    new_master = spawn "#{process.argv[0]}", ["#{path_to_worker}", "--config '#{JSON.stringify config}'"], {cwd:process.cwd()}
